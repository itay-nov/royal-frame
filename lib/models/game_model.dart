...
class GameState {
  ...
  factory GameState.newGame({int? seed}) {
    ...
    return GameState._(
      ...
      totalCardsDrawn: 1, // קלף ראשון כבר נמשך
    );
  }
  ...
  void _evaluatePhaseAfterChange() {
    if (isWinConditionMet) {
      phase = Phase.winner;
      endTime ??= DateTime.now(); // מקפיאים את הזמן!
      return;
    }
  ...
  bool tryPlaceAt(int index, {bool godMode = false}) {
    ...
    // ניקוד על הנחת מלוכה!
    if (card.isK && st == SlotType.kingCorner && !isBlocked[index]) { cells[index] = card; score += 100; _afterPlacement(); return true; }
    if (card.isQ && st == SlotType.queenEdge  && !isBlocked[index]) { cells[index] = card; score += 100; _afterPlacement(); return true; }
    if (card.isJ && st == SlotType.jackEdge   && !isBlocked[index]) { cells[index] = card; score += 100; _afterPlacement(); return true; }
  ...
  void _afterPlacement() {
    ...
    // בודק אם המסגרת התמלאה בדיוק עכשיו ושומר את מספר הקלפים!
  ...
  void applyInstantWin() {
    ...
    score = 1200; // Fake perfect base score
    cardsDrawnWhenFrameFilled = 12; // Fake perfect efficiency
    phase = Phase.winner;
    endTime = DateTime.now();
  }
}
