...
class _TutorialOverlayState extends State<TutorialOverlay> {
  ...
  void _updateHoles() {
    ...
    if (widget.isClearPhase) {
      ...
      if (_step == 0) {
        ...
        _holes = [0, 1, 2, 3, 4, 7, 8, 11, 12, 13, 14, 15]
            .map((i) => _getRect(widget.cellKeys[i])).toList();
      } else if (_step == 2) { 
        _holes = [5, 6, 9, 10].map((i) => _getRect(widget.cellKeys[i])).toList();
      } else if (_step == 3) { 
        _holes = [_getRect(widget.gridKey)];
      }
    }
  ...
  Widget build(BuildContext context) {
    ...
    final alignments = widget.isClearPhase
      ...
      : [Alignment.center, Alignment.center, Alignment.bottomCenter, Alignment.bottomCenter];
          ...
          child: Stack(
            children: [
              AnimatedAlign(
                ...
                child: Padding(
                  padding: margins[_step] + const EdgeInsets.symmetric(horizontal: 24),
                  child: Container(
                              ...
                              Row(
                                children: [
                                  TextButton(
                                    ...
                                    child: Text(isHe ? 'דלג' : 'Skip', style: const TextStyle(color: kGoldDark)),
                                  ),
                                  if (!widget.isClearPhase && _step > 0) ...[
                                    const SizedBox(width: 8),
                                    TextButton(
                                      onPressed: _prevStep,
                                      child: Text(isHe ? 'חזור' : 'Back', style: const TextStyle(color: kGoldLight)),
                                    ),
                                  ]
                                ],
                              ),
